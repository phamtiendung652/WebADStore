        ...
        <div class="content">
            <div class="left">
                ...
                <a href="{{ route('get.user.transaction') }}" title="Kiểm tra đơn hàng" rel="nofollow">Kiểm tra đơn
                    hàng</a>
                <a href="{{ route('get.product.compare') }}" id="goToComparePage" style="margin-left: 20px;">
                    <i class="la la-balance-scale"></i> So sánh (<span id="compareCount">0</span>)
                </a>
            </div>
            <script>
                // Script này sẽ chạy ngay sau khi phần tử #compareCount được tạo
                document.addEventListener('DOMContentLoaded', function() {
                    const compareListKey = 'productCompareList';
                    const compareCountSpan = document.getElementById('compareCount');
                    if (compareCountSpan) {
                        const currentList = JSON.parse(localStorage.getItem(compareListKey) || '[]');
                        compareCountSpan.textContent = currentList.length;
                    }
                });
            </script>
            <div class="right">
                ...
                <li class="desktop">
                    <a href="tel:18006005" title="" class="info-user js-show-dropdown">
                        @if (Auth::check() && Auth::user()->avatar)
                            <img src="{{ pare_url_file(Auth::user()->avatar) }}" alt="Ảnh đại diện">
                        @else
            ...
            <div id="menu-main" class="container" style="display: none">
                <ul class="menu-list">
                    @foreach ($categories as $item)
                        <li>
                            <a href="{{ route('get.category.list', $item->c_slug . '-' . $item->id) }}"
                                ...
                                {{-- Các dòng này đã được comment/xóa để không đụng đến children --}}
                    ...
                    @endif --}}
                            ...
                            {{-- Khối submenu này cũng đã được comment/xóa --}}
                            {{-- @if (isset($item->children) && count($item->children))
                    <div class="submenu">
                        <div class="group">
                            <div class="item">

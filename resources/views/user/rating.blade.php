            ...
            <table class="table table-striped">
                <thead>
                    <tr>
                        ...
                        <th scope="col" style="text-align: center;">Sao</th>
                        ...
                        @foreach ($ratings as $key => $rating)
                            <tr>
                                <td style="text-align: center;">
                                    {{ ($ratings->currentPage() - 1) * $ratings->perPage() + ($key + 1) }}</td>
                                <td style="width: 30%;text-align: left">{{ $rating->product->pro_name ?? '[N\A]' }}</td>
                                <td style="text-align: center;">
                                    <div class="ratings">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <span class="{{ $i <= $rating->r_number ? 'active' : '' }}"><i
                                ...
                                <td style="text-align: center;">
                                    <a href="{{ route('get.user.rating.delete', $rating->id) }}"
